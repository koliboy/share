/*
Cloudflare Worker — FileShare + D1 (auto-increment IDs → short URLs)
===================================================================
This single-file Worker implements a free file-sharing site with:
- Tailwind UI homepage with upload drop-zone
- R2 object storage for file blobs
- D1 database for metadata and auto-increment IDs
- Short public links using Base62-encoded numeric IDs (e.g. /f/aZ3)
- Download pages with ad slots, streaming, and policy pages
...
database_id = "<fill-from-dashboard>"
...
-- (Optional) allow NULL short first then backfill, but we do it in one transaction below.
...
<body class="bg-slate-50 text-slate-900 min-h-screen">
  <div class="max-w-4xl mx-auto p-6">
    ...
    <main class="bg-white rounded-xl shadow p-6">
      ...
      <form id="uploadForm" class="border-dashed border-2 border-slate-200 rounded p-6 flex flex-col items-center justify-center" enctype="multipart/form-data">
        ...
        <div class="flex gap-2 mb-4">
          <label class="flex items-center gap-2"><input type="checkbox" id="streamable" checked /> Streamable (audio/video)</label>
        </div>
    ...
    <footer class="text-center text-xs text-slate-500 mt-6">Powered by Cloudflare R2 + D1 • Replace ad placeholders with your ad code</footer>
  </div>
...
<body class="bg-slate-50 text-slate-900 min-h-screen">
  <div class="max-w-3xl mx-auto p-6">
    ...
    <main class="bg-white p-6 rounded shadow">
      ...
      <div class="text-xs text-slate-500">Direct link: <code id="direct">${locationOrPlaceholder()}/d/${sid}</code></div>

      <hr class="my-4" />
      <h3 class="text-sm font-medium mb-2">By using this site you agree to the <a href="/terms" class="underline">Terms</a> and <a href="/privacy" class="underline">Privacy</a>.</h3>
...
function locationOrPlaceholder(){ return location.origin; }
</script>
</body>
</html>`;
}
...
async function handleUpload(request, env){
  ...
  // We'll do this in a transaction-like sequence:
  // 1) Insert row with temp short_id placeholder
  // 2) Compute Base62 short from last_row_id
  // 3) Upload to R2 under files/{id}
  // 4) Update row with definitive short_id + r2_key

  // Step 1: insert placeholder (short_id will be temporary unique token)
  ...
  // Step 2: compute final short id
  const shortId = base62encode(numericId);

  // Step 3: upload blob to R2 under files/{id}
  ...
  await env.DB.prepare(
    "UPDATE files SET short_id=?, r2_key=? WHERE id=?"
...
async function handleDownload(request, env){
  ...
  const objSize = await headR2Size(env, row.r2_key);
  if(objSize == null) return new Response('File missing in storage', { status: 404 });
  ...
  if(range){
    ...
    let end = m[2] === '' ? objSize - 1 : parseInt(m[2],10);
    if(isNaN(start) || isNaN(end) || start > end || end >= objSize) return new Response('Range not satisfiable', { status: 416 });

    const length = end - start + 1;
    headers.set('Content-Range', `bytes ${start}-${end}/${objSize}`);
    headers.set('Content-Length', String(length));

    // Prefer R2 ranged read
